...
def interpolate_poses(Ts: np.ndarray, n_steps: int) -> np.ndarray:
    ...
    assert Ts.shape == (N_TIMESTEPS, 4, 4), (
        f"Ts.shape: {Ts.shape}, expected: ({N_TIMESTEPS}, 4, 4)"
    )
...
def downsample_and_interpolate_poses(
    Ts: np.ndarray, downsample_factor: int
) -> np.ndarray:
    N_TIMESTEPS = Ts.shape[0]
    assert Ts.shape == (N_TIMESTEPS, 4, 4), (
        f"Ts.shape: {Ts.shape}, expected: ({N_TIMESTEPS}, 4, 4)"
    ...
    if interpolated_Ts.shape[0] < N_TIMESTEPS:
        ...
        interpolated_Ts = np.concatenate(
            [interpolated_Ts, interpolated_Ts[-1][None].repeat(extra, axis=0)], axis=0
        )
    elif interpolated_Ts.shape[0] > N_TIMESTEPS:
        extra = interpolated_Ts.shape[0] - N_TIMESTEPS
        interpolated_Ts = interpolated_Ts[:-extra]
    assert interpolated_Ts.shape == (N_TIMESTEPS, 4, 4), (
        f"interpolated_Ts.shape: {interpolated_Ts.shape}, expected: ({N_TIMESTEPS}, 4, 4)"
    )
    return Ts
...
def solve_fingertip_ik(
    ...
    _hand_joint_names = get_joint_names(hand_pb)
...
def create_transformed_keypoint_to_xyz(hand_json: dict, T_W_C: np.ndarray) -> dict:
    ...
    kpt_map["PALM_TARGET"] = mean_wrist
            ...
            if args.retarget_robot_using_object_relative_pose and i >= idx_lifted:
                ...
                if i == idx_lifted:
                    ...
                    T_R_P_lifted = compute_current_T_R_P(
                        arm_pk_chain=arm_pk_chain, q_arm=q_arm_lifted
                    )
                    T_W_P_lifted = T_W_R @ T_R_P_lifted
                    T_W_O_lifted = T_W_Os[idx_lifted]
                    T_O_P_lifted = np.linalg.inv(T_W_O_lifted) @ T_W_P_lifted
