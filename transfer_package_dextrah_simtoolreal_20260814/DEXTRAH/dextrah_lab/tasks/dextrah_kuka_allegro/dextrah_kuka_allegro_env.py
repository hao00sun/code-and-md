...
def set_prim_attribute_value(prim_path: str, attribute_name: str, value):
    """Set a USD prim attribute without importing deprecated Isaac Sim utils."""
    stage = omni.usd.get_context().get_stage()
    attr = stage.GetPrimAtPath(prim_path).GetAttribute(attribute_name)
    attr.Set(value)


def physx_view_to_torch(value, device: str):
    """Convert PhysX view outputs from torch or Warp arrays to torch tensors."""
    if isinstance(value, torch.Tensor):
        return value.to(device=device)
    return warp.torch.to_torch(value).to(device=device)
...
class DextrahKukaAllegroEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: DextrahKukaAllegroEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # joint limits
        if hasattr(self.robot.data, "joint_pos_limits"):
            joint_pos_limits = self.robot.data.joint_pos_limits
            if hasattr(joint_pos_limits, "torch"):
                joint_pos_limits = joint_pos_limits.torch
            joint_pos_limits = joint_pos_limits.to(self.device)
        else:
        ...
        if hasattr(self.robot, "write_joint_friction_coefficient_to_sim_index"):
    ...
    def get_object_names(self, objects_dir, apply_subset=True):
        ...
        if apply_subset and self.cfg.use_object_subset:
            if self.cfg.object_subset_size <= 0:
                raise ValueError("object_subset_size must be positive when use_object_subset=True")
            if self.cfg.object_subset_start_index < 0:
                raise ValueError("object_subset_start_index must be non-negative")
            start = self.cfg.object_subset_start_index
            end = start + self.cfg.object_subset_size
            sub_dirs = sub_dirs[start:end]
            if len(sub_dirs) == 0:
                raise ValueError(
                    "Object subset is empty. Check object_subset_start_index and object_subset_size."
                )

        return sub_dirs

    def find_num_unique_objects(self, objects_dir):
        return len(self.get_object_names(objects_dir, apply_subset=False))
    ...
    def _setup_scene(self):
        ...
        # IsaacLab 3.x clones env prims during InteractiveScene initialization.
        # IsaacLab 2.x still exposes clone_environments for direct workflows.
    ...
    def _setup_objects(self):
        ...
        all_sub_dirs = self.get_object_names(self.cfg.objects_dir, apply_subset=False)
        sub_dirs = self.get_object_names(self.cfg.objects_dir, apply_subset=True)

        self.num_unique_objects = len(all_sub_dirs)
        self.num_loaded_objects = len(sub_dirs)

        # This creates a 1D tensor array of length self.num_envs which provides
        # the object ID over all envs. Keep IDs in the full object vocabulary so
        # subset runs remain observation-compatible with full-object runs.
        # local_rank = int(os.getenv("LOCAL_RANK", 0))
        # self.multi_object_idx = torch.remainder(
        #     torch.arange(self.num_envs)+self.num_envs*local_rank,
        #     self.num_unique_objects
        # ).to(self.device)
        loaded_object_indices = torch.tensor(
            [all_sub_dirs.index(object_name) for object_name in sub_dirs],
            device=self.device,
            dtype=torch.long,
        )
        self.object_choice_idx = torch.remainder(torch.arange(self.num_envs), self.num_loaded_objects).to(self.device)
        self.multi_object_idx = loaded_object_indices[self.object_choice_idx]
...
#                torch.ones(self.num_envs, 1, device=self.device)
        ...
        object_load_log_interval = max(1, self.num_envs // 20)
        object_set_name = (
            f"subset start={self.cfg.object_subset_start_index}, size={self.cfg.object_subset_size}"
            if self.cfg.use_object_subset
            else "full set"
        )
        print(
            f"Loading {self.num_envs} object instances from "
            f"{objects_full_path} ({self.num_loaded_objects}/{self.num_unique_objects} unique objects, {object_set_name})"
        )

        for i in range(self.num_envs):
            # TODO: check to see that the below config settings make sense
            object_name = sub_dirs[int(self.object_choice_idx[i].item())]
            ...
            if i == 0 or (i + 1) % object_load_log_interval == 0 or i == self.num_envs - 1:
                progress = 100.0 * (i + 1) / self.num_envs
                print(
                    f"Loading objects: {i + 1}/{self.num_envs} "
                    f"({progress:.1f}%) latest={object_name}"
                )
    ...
    def _get_dones(self) -> torch.Tensor:
        ...
        if self.cfg.distillation or self.cfg.reset_on_success_timeout:
...
#        )
        ...
        self.object_mass = physx_view_to_torch(self.object.root_physx_view.get_masses(), self.device)
        ...
        self.robot_dof_stiffness = physx_view_to_torch(
            self.robot.root_physx_view.get_dof_stiffnesses(),
            self.device,
        )
        self.robot_dof_damping = physx_view_to_torch(
            self.robot.root_physx_view.get_dof_dampings(),
            self.device,
        )
        self.robot_material_props =\
            physx_view_to_torch(self.robot.root_physx_view.get_material_properties(), self.device).view(self.num_envs, -1)
...
#            False)
    ...
    def _compute_intermediate_values(self):
        ...
        incoming_joint_forces = physx_view_to_torch(
            self.robot.root_physx_view.get_link_incoming_joint_force(),
            self.device,
        )
        self.hand_forces = incoming_joint_forces[:, self.hand_bodies]
        self.hand_forces =\
            self.hand_forces.view(self.num_envs, self.num_hand_bodies * 6)
        # Query the measured torque on the joints
        self.measured_joint_torque = physx_view_to_torch(
