...
def make_sim_cfg(*, sim_dt, render_interval):
    """Create SimulationCfg across IsaacLab 2.x and 3.x PhysX field names."""
    kwargs = dict(
        dt=sim_dt,
    ...
    try:
        return SimulationCfg(**kwargs, physics=physx_cfg)
    except TypeError:
        return SimulationCfg(**kwargs, physx=physx_cfg)
...
class DextrahKukaAllegroEnvCfg(DirectRLEnvCfg):
    ...
    object_subset_size = 64
    object_subset_start_index = 0
    ...
    success_timeout = 2.
    reset_on_success_timeout = False
    # num_observations = 94
    distillation = False
    ...
    # simulation
    sim: SimulationCfg = make_sim_cfg(sim_dt=sim_dt, render_interval=num_sim_steps_to_render)
    # robot
