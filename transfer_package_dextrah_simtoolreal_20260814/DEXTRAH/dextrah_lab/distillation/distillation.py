...
class Dagger:
    def __init__(self, env, config, summaries_dir, nn_dir):
        ...
        self.resume_optimizer = self.config["student"].get("resume_optimizer", False)
        self.resume_iteration = self.config["student"].get("resume_iteration", None)
        self.beta_schedule = self.config.get("beta_schedule", "time_staged")
        self.beta_start = self.config.get("beta_start", 1.0)
        self.beta_end = self.config.get("beta_end", 0.4)
        self.beta_warmup_iters = self.config.get("beta_warmup_iters", 30_000)
        self.beta_decay_end_iter = self.config.get("beta_decay_end_iter", 180_000)
        self.beta_step = self.config.get("beta_step", 0.05)
        self.beta_success_target = self.config.get("beta_success_target", 0.30)
        self.beta_hold_iters = self.config.get("beta_hold_iters", 5_000)
        ...
        self.play_teacher = self.config.get("play_teacher", False)
        if self.play_policy is True:
            self.step_student_actions = not self.play_teacher
        ...
        self.success_rate_sum = 0.0
        self.success_rate_window = deque(maxlen=100)
        self.mean_success_all = 0.0
        self.mean_success_last_100 = 0.0
    ...
    def distill(self):
        ...
        log_counter = self.start_iteration
        self.current_iteration = log_counter
        total_loss = 0.
        ...
        best_success_region = 0.0
        last_beta_decrease_iter = self.start_iteration

        while log_counter < num_iters:
            if self.beta_schedule == "success" and (log_counter % 240 == 0 or num_iters_since_beta_dec > 1_000):
                num_iters_since_beta_dec = 0
                perf = float(self.ov_env.in_success_region.float().mean().cpu().numpy())
                best_success_region = max(best_success_region, perf)
                beta = self.get_success_beta(best_success_region)
            elif self.beta_schedule == "success_hold" and log_counter % 240 == 0:
                perf = float(self.ov_env.in_success_region.float().mean().cpu().numpy())
                best_success_region = max(best_success_region, perf)
                target_beta = self.get_success_beta(best_success_region)
                enough_time_elapsed = (log_counter - last_beta_decrease_iter) >= self.beta_hold_iters
                if target_beta < beta and enough_time_elapsed:
                    beta = max(target_beta, beta - self.beta_step)
                    beta = self.quantize_beta(beta)
                    last_beta_decrease_iter = log_counter
            elif self.beta_schedule == "time_staged":
                beta = self.get_time_staged_beta(log_counter)
            if self.play_policy:
            ...
            if beta is None:
                ...
                teacher_inds = torch.rand(self.num_envs, device=self.device) < beta
            ...
            if not self.play_policy:
                ...
                if self.rank == 0 and self.save_every > 0 and log_counter % self.save_every == 0:
    ...
    def update_success_metrics(self):
        success_rate = float(self.ov_env.in_success_region.float().mean().detach().cpu().item())
        self.success_step_count += 1
        self.success_rate_sum += success_rate
        self.success_rate_window.append(success_rate)
        self.mean_success_all = self.success_rate_sum / self.success_step_count
        self.mean_success_last_100 = sum(self.success_rate_window) / len(self.success_rate_window)

    def quantize_beta(self, beta):
        if self.beta_step > 0:
            beta = round(beta / self.beta_step) * self.beta_step
        beta_min = min(self.beta_start, self.beta_end, 0.0)
        beta_max = max(self.beta_start, self.beta_end, 1.0)
        return min(max(beta, beta_min), beta_max)

    def get_success_beta(self, success_region):
        beta = max(1.0 - success_region / self.beta_success_target, 0.0)
        return self.quantize_beta(beta)

    def get_time_staged_beta(self, iteration):
        if iteration < self.beta_warmup_iters:
            return self.beta_start

        decay_span = max(1, self.beta_decay_end_iter - self.beta_warmup_iters)
        progress = min(max((iteration - self.beta_warmup_iters) / decay_span, 0.0), 1.0)
        beta = self.beta_start + progress * (self.beta_end - self.beta_start)
        if self.beta_step > 0:
            beta = math.ceil(beta / self.beta_step) * self.beta_step
        beta_min = min(self.beta_start, self.beta_end)
        beta_max = max(self.beta_start, self.beta_end)
        return min(max(beta, beta_min), beta_max)
    ...
    def log_information(self, log_counter, total_loss, aux_loss=None, beta=None):
        ...
        if self.game_rewards.current_size > 0:
            ...
            for i in range(self.value_size):
                ...
                self.writer.add_scalar(
                    "mean_success_last_100", self.mean_success_last_100, self.frame
                )
                self.writer.add_scalar(
                    "mean_success_all", self.mean_success_all, self.frame
                )
                if self.use_wandb:
                    wandb.log({
                        "in_success_region": perf,
                        "mean_success_last_100": self.mean_success_last_100,
                        "mean_success_all": self.mean_success_all,
        ...
        if log_counter % 10 == 0:
            ...
            if self.game_rewards.current_size > 0:
                ...
                print("\tmean_success_last_100: ", self.mean_success_last_100)
                print("\tmean_success_all: ", self.mean_success_all)
    ...
    def get_actions(self, obs, policy_type):
        ...
        if policy_type == "student":
            ...
            mus = res_dict["mus"]
            sigmas = res_dict["sigmas"]
        if policy_type == "teacher" and self.play_teacher:
            selected_action = mus.squeeze()
        else:
    ...
    def set_weights(self, ckpt, policy_type):
        ...
        if policy_type == "student":
            ...
            if self.resume_optimizer:
                ...
                if self.resume_iteration is not None:
                    self.start_iteration = self.resume_iteration
                    self.frame = self.start_iteration * self.num_envs
                elif 'iteration' in weights:
                    self.start_iteration = weights['iteration']
                elif self.num_envs > 0:
                    self.start_iteration = self.frame // self.num_envs
        else:
    ...
    def save(self, filename):
        ...
        state['iteration'] = getattr(self, 'current_iteration', self.start_iteration)
