...
parser.add_argument("--save_every", type=int, default=None, help="Student checkpoint save interval in iterations.")
...
parser.add_argument("--resume_optimizer", action="store_true", default=False, help="Resume optimizer and frame from student checkpoint")
parser.add_argument("--resume_iteration", type=int, default=None, help="Iteration to resume from when changing num_envs")
parser.add_argument("--play_policy", type=bool, default=False, help="Play a distilled policy.")
parser.add_argument("--play_teacher", action="store_true", default=False, help="Use teacher actions when playing in distillation mode")
parser.add_argument(
    "--beta_schedule",
    choices=["time_staged", "success", "success_hold"],
    default="time_staged",
    help="Teacher-action beta schedule for distillation.",
)
parser.add_argument("--beta_start", type=float, default=1.0, help="Initial teacher-action probability.")
parser.add_argument("--beta_end", type=float, default=0.4, help="Final teacher-action probability for time_staged schedule.")
parser.add_argument("--beta_warmup_iters", type=int, default=30_000, help="Iterations to keep beta at beta_start.")
parser.add_argument("--beta_decay_end_iter", type=int, default=180_000, help="Iteration where beta reaches beta_end.")
parser.add_argument("--beta_step", type=float, default=0.05, help="Quantization step for beta.")
parser.add_argument("--beta_success_target", type=float, default=0.30, help="Success rate that maps beta to zero for success schedules.")
parser.add_argument("--beta_hold_iters", type=int, default=5_000, help="Minimum iterations to hold beta between decreases for success_hold.")
...
def main(env_cfg, agent_cfg: dict):
    ...
    elif not args_cli.play_policy or args_cli.play_teacher:
    ...
    if rank == 0:
        train_dir = "/mnt/bigdata/SUN_ht/runs/dextrah"
        experiment_name = (
    ...
    dagger_config = {
        "student": {
            ...
            "resume_optimizer": args_cli.resume_optimizer,
            "resume_iteration": args_cli.resume_iteration,
        },
        "teacher": {
            "cfg": teacher_cfg,
            "ckpt": teacher_ckpt,
        ...
        "beta_warmup_iters": args_cli.beta_warmup_iters,
        "beta_decay_end_iter": args_cli.beta_decay_end_iter,
        "beta_step": args_cli.beta_step,
        "beta_success_target": args_cli.beta_success_target,
        "beta_hold_iters": args_cli.beta_hold_iters,
    }
    ...
    if args_cli.max_iterations is not None:
        dagger.num_iters = args_cli.max_iterations
    if args_cli.save_every is not None:
        dagger.save_every = args_cli.save_every
    dagger.distill()
    if rank == 0 and not args_cli.play_policy:
        dagger.save(os.path.join(nn_dir, "sh_dist_scratch_cnn"))

    try:
        env.close()
    except Exception as err:
        print(f"[WARN] Failed to close env cleanly: {err}")
    if dist.is_available() and dist.is_initialized():
        try:
            dist.destroy_process_group()
        except Exception as err:
            print(f"[WARN] Failed to destroy process group cleanly: {err}")
...
if __name__ == "__main__":
    try:
        ...
        # Give Isaac Sim a chance to release plugins, cameras, and Fabric/PhysX resources.
