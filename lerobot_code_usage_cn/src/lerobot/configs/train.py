...
class TrainPipelineConfig(HubMixin):
    ...
    # Optional directory where the training log file is written. Supports the same {timestamp} and {job_name}
    # placeholders as output_dir.
    ...
    def validate(self) -> None:
        ...
        if self.output_dir is not None:
            self.output_dir = Path(str(self.output_dir).format(timestamp=timestamp, job_name=self.job_name))

        if self.log_dir is not None:
            self.log_dir = Path(str(self.log_dir).format(timestamp=timestamp, job_name=self.job_name))
