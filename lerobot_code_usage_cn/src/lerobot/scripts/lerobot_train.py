...
def update_policy(
    ...
    # Scale the loss so accumulated gradients match a single larger batch update.
    accelerator.backward(loss / gradient_accumulation_steps)

    if do_optimizer_step:
        # Clip gradients if specified, once after all accumulation micro-steps.
        if grad_clip_norm > 0:
...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    if cfg.log_dir is not None:
        ...
        log_file = cfg.log_dir / "train.log"
    ...
    if not cfg.is_reward_model_training and processor_pretrained_path is not None:
        dataset_stats_for_policy = rename_stats(dataset.meta.stats, cfg.rename_map) if cfg.rename_map else dataset.meta.stats
        preprocessor_overrides = {
            "device_processor": {"device": device.type},
            "normalizer_processor": {
                "stats": dataset_stats_for_policy,
    ...
    if is_main_process:
        ...
        logging.info(
            ...
            f"{cfg.batch_size} x {cfg.gradient_accumulation_steps} accumulation x {num_processes} "
            f"= {effective_bs}"
        )
    ...
    for _ in range(step, cfg.steps):
        ...
        for accumulation_i in range(cfg.gradient_accumulation_steps):
            start_time = time.perf_counter()
            batch = next(dl_iter)
            ...
            train_tracker, micro_output_dict = update_policy(
                ...
                do_optimizer_step=accumulation_i == cfg.gradient_accumulation_steps - 1,
            )
            if micro_output_dict:
