"""Serve a LeRobot MultiTaskDiT Flow Matching Policy while a remote client executes actions.
...
def build_metadata(policy, policy_path, action_mode):
    ...
    return {
        "model": "lerobot_multi_task_dit_flow_matching",
        "policy_path": policy_path,
        "action_mode": action_mode,
        "freeze_right_arm": bool(getattr(policy, "_server_freeze_right_arm", False)),
...
def log_result(step, reply, diagnostics, request_interval_s, print_action):
    ...
    if "safe_action" in diagnostics:
        print(
            f"[SERVER] safety freeze_right_arm={diagnostics.get('freeze_right_arm')} "
            f"right_arm_frozen={diagnostics.get('right_arm_frozen')} "
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Freeze right-arm action dimensions 7:14 to the current right-arm state. "
            "Enabled by default because the current training dataset did not drive the right arm."
        ),
    )
    parser.add_argument(
        "--clip_to_dataset_action_range",
...
def main():
    ...
    policy._server_freeze_right_arm = args.freeze_right_arm  # noqa: SLF001
    ...
    print("[SERVER] freeze_right_arm:", args.freeze_right_arm)
